...
from llava_trainer_2stages import LLaVATrainer
...
wandb.login(key="4f4b3d8a51a979a9063d7c842ee7e6dcd4be7d25")
...
def add_diffusion_noise(image_tensor, noise_step):
    ...
    alphas_prod_p = torch.cat(
        [torch.tensor([1]).float(), alphas_prod[:-1]], 0
    )  # p for previous
...
def preprocess(
    ...
    if chosen == 1:
        return dict(
            chosen_input_ids=input_ids, chosen_labels=targets, prompt=conversations[0]
        )
    else:
        return dict(rejected_input_ids=input_ids, rejected_labels=targets)
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0]:
            ...
            if self.list_data_dict[i].get("rejected_noised") == 1:
                rejected_image = generate_gaussian_noise(image,mean=0.0, stddev=1)
                # tqdm.write('generating gaussian noise for the image')
            elif self.list_data_dict[i].get("chosen_noised") == 1:
                image = generate_gaussian_noise(image,mean=0.0, stddev=1)
                rejected_image=image
                tqdm.write('generating gaussian noise for chosen image')
            else:
                rejected_image=image
            if self.data_args.image_aspect_ratio == "pad":
                ...
                rejected_image = expand2square(
                    rejected_image, tuple(int(x * 255) for x in processor.image_mean)
                )
                rejected_image = processor.preprocess(rejected_image, return_tensors="pt")[
                    "pixel_values"
                ][0]
        ...
        if isinstance(i, int):
            data_dict = dict(
                chosen_input_ids=data_dict["chosen_input_ids"][0],
        ...
        if "image" in sources[0]:
            ...
            rejected_sources = copy.deepcopy(
                [e["rejected_conversations"] for e in sources]
            )
        ...
        if isinstance(i, int):
            rejected_data_dict = dict(
                rejected_input_ids=rejected_data_dict["rejected_input_ids"][0],
        ...
        if "image" in self.list_data_dict[i]:
            ...
            data_dict["rejected_images"] = rejected_image
        elif self.data_args.is_multimodal:
            # image does not exist in the data, but the model is multimodal
            crop_size = self.data_args.image_processor.crop_size
            data_dict["images"] = torch.zeros(
                3, crop_size["height"], crop_size["width"]
            )
            data_dict["rejected_images"] = torch.zeros(
                3, crop_size["height"], crop_size["width"]
            )
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        chosen_input_ids, chosen_labels = tuple(
        ...
        rejected_input_ids, rejected_labels = tuple(
            [instance[key] for instance in instances]
        ...
        if "images" in instances[0]:
            ...
            rejected_images=[instance["rejected_images"] for instance in instances]
            if all(x is not None and x.shape == images[0].shape for x in images):
                batch["images"] = torch.stack(images)
                batch["rejected_images"]=torch.stack(rejected_images)
            else:
                batch["images"] = images
                batch["rejected_images"]=rejected_images
...
def train():
    ...
    if training_args.lora_enable:
        ...
        if model_args.lora_checkpoint_path:
            model_path = model_args.lora_checkpoint_path
            print(f"Loading additional LLaVA weights from {model_path}...")
            ...
            # 加载 non-lora trainables
            model.load_state_dict(non_lora_trainables, strict=False)
            print("Loaded additional non-LoRA weights.")

            
            print(f"Loading LoRA weights from {model_args.lora_checkpoint_path}")
            ...
            # 如果没有指定 lora_checkpoint_path，继续训练 LoRA
