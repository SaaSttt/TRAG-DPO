...
if __name__ == "__main__":
    ...
    datasets_train_json_path    = "/data/datasets/zhuhaoran/CDDMBench/datasets/train.json"
    datasets_val_json_path      = "/data/datasets/zhuhaoran/CDDMBench/datasets/quiry.json"
    ...
    # 加载文本编码器
    text_pretrained_path = "model_data/ViT-B-16-OpenAI.pth"

    import numpy as np
    import torch

    # 1. 读取 .npz 文件
    pretrained = np.load("model_data/ViT-B_16.npz")
    model.visual.load_from(pretrained)

    def load_text_encoder_weights(model, pretrained_path):
        ...
        temp_dict = {k: state_dict[k] for k in text_keys if k in model_dict and state_dict[k].shape == model_dict[k].shape}
        missing_keys = [k for k in text_keys if k not in temp_dict]
        print(f"成功加载: {len(temp_dict)} 层参数")
        print(f"跳过的参数（名称或维度不匹配）: {missing_keys}")
    ...
    # 加载文本编码器（根据类型区分）
    if model.bert_type == "openai":
        load_text_encoder_weights(model, text_pretrained_path)
