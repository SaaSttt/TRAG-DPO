...
class ClipDataset(data.Dataset):
    def __init__(self, input_shape, lines, datasets_path, random, autoaugment_flag=True):
        ...
        for img_id, ann in enumerate(self.lines):
            ...
            self.label.append(torch.tensor(ann['label'], dtype=torch.float32))
            self.img2txt[img_id] = []
...
def dataset_collate(batch):
    ...
    images      = []
    captions    = []
    for image, caption,label in batch:
        labels.append(label)
        images.append(image)
...
def dataset_collate2(batch):
    labels = []
    images      = []
    captions    = []
    for label,image, caption in batch:
        labels.append(label)
        images.append(image)
        captions.append(caption)
        
    images      = torch.from_numpy(np.array(images)).type(torch.FloatTensor)
    return labels,images, captions


class ClipDataset2(data.Dataset):
    def __init__(self, input_shape, lines, datasets_path, random, autoaugment_flag=True):
        self.input_shape    = input_shape
        self.lines          = lines
