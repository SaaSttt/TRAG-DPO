...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if config.split == 'non-overlap':
            # Update n_patches for 256x256 image and 16x16 patch size
        ...
        # Position embeddings must match the number of patches + 1 (for CLS token)
    ...
    def forward(self, x):
        ...
        # Ensure position_embeddings matches x's sequence length
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False):
        ...
        self.part_head = Linear(config.hidden_size, 512)
        self.part_head1 = Linear(config.hidden_size, 10)

    def forward(self, x, labels=None):
        part_tokens = self.transformer(x)
        # print(part_tokens.shape)
        part_logits1 = self.part_head1(part_tokens[:, 0])
        part_logits = self.part_head(part_tokens)
        part_logits = part_logits.mean(dim=1)
        # part_logits = part_logits / part_logits.norm(dim=-1, keepdim=True)
        # part_logits = part_tokens

        if labels is not None:
            if isinstance(labels, list):
                labels = torch.stack([torch.tensor(l) if not isinstance(l, torch.Tensor) else l for l in labels])

            if labels.ndim > 1:  # one-hot 向量
            ...
            part_loss = loss_fct(part_logits1.view(-1, self.num_classes), labels.view(-1))
